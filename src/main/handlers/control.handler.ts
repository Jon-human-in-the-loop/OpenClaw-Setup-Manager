...
export function registerControlHandlers(): void {
  ...
  /**
   * control:container-logs — Fetch historical container logs from SQLite.
   */
  ipcMain.handle("control:container-logs", async (_, limit = 200): Promise<ContainerLogEntry[]> => {
    return getContainerLogs(CONTAINER_NAME, limit);
  });
